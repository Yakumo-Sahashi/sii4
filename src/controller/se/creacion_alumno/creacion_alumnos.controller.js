let input=['tk_frm','numero_control','no_control',
           'apellido_paterno','apellido_materno','nombres','fecha_nacimiento','telefono','correo_electronico','curp','escritura_manual','codigo_postal','estado','alcaldia','colonia','calle','no_exterior','no_interior','carrera_reticula','especialidad','periodo_ingreso','periodo_ingreso_id','periodos_revalidados','cb_revalidado','plan_est','plan_estudios','escuela_alumno'
];
$(document).ready(() => {
    let numeros_control = "";
    let posicion = 0;
    ...
    const mostrarDatosControl = () => {
		...
		fetch(`app/Controllers/InformacionCatalogos.php`, {
				method: `POST`,
				body: datos
    ...
    const crear_alumno = () => {
        ...
        datos.append('funcion', "crear_alumno");
        fetch(`app/Controllers/CreacionAlumno.php`, {
            method: `POST`,
            body: datos
        }).then(respuesta => respuesta.json())
        .then(respuesta => {
            bootstrap.Itma2.end_loader();
            if (respuesta[0] == "1") {
                $(`[name=alumno]`).val(respuesta[2]);
                $("#staticBackdrop").modal("show");
                $(`#cerrar_modal`).hide(); 
                msj_exito(`${respuesta[1]}`);		
                obtener_semestre_se();	
                $('#btn_control_manual').addClass('btn-danger').removeClass('btn-primary');   
                $('#btn_control_manual').html('<i class="fa-solid fa-lock"></i>');  
                $('[name=no_control]').attr('readonly','true');
                $('#incremento_decr').removeClass('d-none');		
            } else {
    ...
    const validar_vacios_datos_escolares = () => {
        let campos = ['carrera_reticula', 'especialidad', 'periodo_ingreso', 'plan_estudios', 'nivel_escolar', 'estatus_alumno','escuela_alumno'];
        if (validar_campo(campos, 'vacios')) {
            crear_alumno();
        }
    }
    ...
    $('#btn_control_manual').on('click',() => {
        if($('[name=numero_control]').val() == 'esc'){
            $('#btn_control_manual').addClass('btn-danger').removeClass('btn-primary');   
            $('#btn_control_manual').html('<i class="fa-solid fa-lock"></i>');  
            $('[name=no_control]').attr('readonly','true');
            $('#incremento_decr').removeClass('d-none');
            ...
            $('#btn_control_manual').removeClass('btn-danger').addClass('btn-primary');   
            $('#btn_control_manual').html('<i class="fa-solid fa-lock-open"></i>');  
            $('[name=no_control]').removeAttr('readonly');
            $('[name=no_control]').val('');
            $('#incremento_decr').addClass('d-none');
            $('[name=numero_control]').val('esc');
        }
    });
});