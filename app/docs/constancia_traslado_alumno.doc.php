<?php
    ...
    $pdf->Cell(50,5,utf8_decode('C.'.$_POST['nombre_director']),0,1,'L');
    $pdf->Cell(50,5,utf8_decode('Directo'.$_POST['dir'].' del Instituto Tecnológico de '.$_POST['instituto']),0,1,'L');
    $pdf->Ln(5);
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(127,5,utf8_decode('Por este conducto, le comunico que no existe incoveniente por parte nuestra, para que el'),0,0,'L',true);
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(65,5,mb_strtoupper(utf8_decode('C.'.$consulta->nombre_persona.' '.$consulta->apellido_paterno.' '.$consulta->apellido_materno),'UTF-8'),0,1,'L',true);
    $pdf->SetFont('Arial','',9);
    $pdf->MultiCell(192,5,utf8_decode('con número de control '.$consulta->numero_control.' de la carrera '.$consulta->nombre_carrera.', con clave oficial '.$consulta->clave_oficial.','),0,'FJ',1);
    $pdf->MultiCell(192,5,utf8_decode('se traslade al instituto Tecnológico a su cargo, para lo cual se anexa el historial académico detallado por semestres.'),0,'FJ',1);
