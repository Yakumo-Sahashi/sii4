<?php
	...
	$cantidad = TablaHistoriaAlumno::select('t_historia_alumno.fk_numero_control')->join('t_alumno','t_historia_alumno.fk_numero_control','t_alumno.fk_numero_control')->where('t_historia_alumno.fk_periodo_escolar',$_POST['periodo_bloque'])->where('t_alumno.fk_cat_carrera',$_POST['carrera_bloque'])->orderBy('t_historia_alumno.fk_numero_control','asc')->get();
	$temp = 0;
	$periodo = TablaPeriodoEscolar::select('identificacion_corta')->where('id_periodo_escolar',$_POST['periodo_bloque'])->first();
	if(count($cantidad) > 0){
		foreach($cantidad as $id_datos){
			if($id_datos->fk_numero_control == $temp){
				continue;
			}
			$temp = $id_datos->fk_numero_control;
			$pdf->AliasNbPages();
			$pdf->AddPage();	
			$alumno = TablaAlumno::select('apellido_paterno','apellido_materno','nombre_persona','numero_control','t_cat_carrera.nombre_carrera','t_cat_especialidad.especialidad','t_alumno.semestre','t_alumno.periodos_revalidados')->join('t_persona','t_alumno.fk_persona','t_persona.id_persona')->join('t_numero_control','t_alumno.fk_numero_control','t_numero_control.id_numero_control')->join('t_cat_carrera','t_alumno.fk_cat_carrera','t_cat_carrera.id_cat_carrera')->join('t_cat_especialidad','t_alumno.fk_cat_especialidad','t_cat_especialidad.id_cat_especialidad')->where('t_alumno.fk_numero_control',$id_datos->fk_numero_control)->first();
	...
	$pdf->Output('D', 'boletas_bloque.pdf');
?>