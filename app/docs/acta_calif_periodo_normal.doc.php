<?php
	...
	class PDF extends FPDF{
		function periodo(){
			$consulta = TablaPeriodoEscolar::select('identificacion_larga')->where('id_periodo_escolar',$_POST['periodo_periodo_normal'])->first();
    ...
    $consulta = TablaPeriodoEscolar::select('identificacion_corta')->where('id_periodo_escolar',$_POST['periodo_periodo_normal'])->first();
	$materias = TablaGrupo::select('id_grupo','t_grupo.fk_cat_materias','nombre_grupo','nombre_persona','apellido_paterno','apellido_materno','nombre_completo_materia','alumnos_inscritos','clave_oficial','t_cat_organigrama.descripcion')->join('t_personal','t_grupo.fk_personal','t_personal.id_personal')->join('t_persona','t_personal.fk_persona','t_persona.id_persona')->join('t_cat_materias','t_grupo.fk_cat_materias','t_cat_materias.id_cat_materias')->join('t_cat_organigrama','t_cat_materias.fk_cat_organigrama','t_cat_organigrama.id_cat_organigrama')->where('t_grupo.fk_periodo',$_POST['periodo_periodo_normal'])->orderBy('t_cat_materias.fk_cat_organigrama','asc')->get();
	...
	if(count($materias) > 0){
		foreach($materias as $materia){
			...
			if(strlen($materia->descripcion) > 47){
		...
		/* $pdf->Cell(30,5,u tf8_decode('San Juan Tepenahuac, Alcaldia Milpa Alta, CDMX a '.$dia.' de '.$mes[$me].' del '.$year),0,1,'L'); */
		$pdf->SetX(11);
		$pdf->Cell(192,50,utf8_decode('NO HAY ACTAS DE CALIFICACIONES'),0,1,'C');

	}
	
	$pdf->Output('D', 'acta_calif_periodo_normal.pdf');
?>
