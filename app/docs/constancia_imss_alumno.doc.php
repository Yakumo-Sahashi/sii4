<?php
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->MultiCell(192,5,utf8_decode('Por este conducto me dirijo a usted de la manera más atenta para hacer de su conocimiento que el alumno:'),0,'FJ',1);
    $pdf->SetFont('Arial','B',9);
    $pdf->MultiCell(192,6,mb_strtoupper(utf8_decode($consulta->nombre_persona.' '.$consulta->apellido_paterno.' '.$consulta->apellido_materno),'UTF-8'),0,'C',0);
    $pdf->SetFont('Arial','',9);
    $pdf->MultiCell(192,5,utf8_decode('De la carrera '.$consulta->nombre_carrera.', con el número de control '.$consulta->numero_control.' y número de'),0,'FJ',1);
    $pdf->MultiCell(192,5,utf8_decode('afiliación al I.M.S.S. '.$_POST['afiliacion'].'. Clinica #'.$_POST['clinica'].' cuenta con servicio médico como se hace constar en nuestro archivo.'),0,'FJ',1);
    
    $pdf->Ln(4);
    $pdf->MultiCell(145,5,utf8_decode('Por las facilidades que tenga en brindar a mi atento escrito, le participo mi agradecimiento.'),0,'FJ',1);
    ...
    $pdf->Output('D', 'constancia_IMSS.pdf');
?>