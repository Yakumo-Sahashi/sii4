<?php
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(40,5,utf8_decode('CONSTANCIA DE AVANCE'),0,1,'L');
    $pdf->Ln(5);
    ...
    $pdf->SetFont('Arial','',9);
    $pdf->MultiCell(192,5,utf8_decode('con número de control '.$consulta->numero_control.', fue alumno del periodo '.$periodo->identificacion_larga.', cursando el semestre '.($consulta->semestre + $consulta->periodos_revalidados).' de la carrera de '.$consulta->nombre_carrera.' y a la fecha ha acreditado '.$consulta->creditos_aprobados.' créditos de un total de 260 que integran el plan'),0,'FJ',1);

    $pdf->MultiCell(192,5,utf8_decode('de estudios '.$consulta->clave_reticula.'. Por lo que su porcentaje de avance es de '.($consulta->creditos_aprobados == 0 ? 0 : $consulta->creditos_aprobados/260).'.'),0,'',1);
    ...
    $pdf->Output('D', 'constancia_avance.pdf');
?>