...
use model\TablaHorarioPeronal;
	...
	class RhHorarios {
		...
		static function reordenar_fecha($fecha){
			$fecha = explode("-",$fecha);
			$formato = strtotime($fecha[2]."-".$fecha[1]."-".$fecha[0]);
		...
		static function mostar_horarios(){
			...
			/* $horario_final = array();
			for($i = 0; $i < count($consulta); $i+=2){
				array_push($horario_final,[$consulta[$i]['dia']=>['hora_entrada'=>$consulta[$i]['hora_inicio'],'hora_salida'=>$consulta[$i]['hora_fin']],'nombre'=>$consulta[$i]['nombre_persona'].' '.$consulta[$i]['apellido_paterno'].' '.$consulta[$i]['apellido_materno'],'checador_id'=> $consulta[$i]['checador_id']]);			
			}
			echo json_encode($horario_final); */
			$i = 0;
